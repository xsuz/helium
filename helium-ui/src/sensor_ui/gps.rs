...
impl GPSUI {
    ...
    fn pixel2gps(&self, x: f64, y: f64) -> (f64, f64) {
        // https://www.trail-note.net/tech/coordinate/
        let lon = 180.0
            * ((x + 256.0 * self.map_x as f64) / 2.0_f64.powf(self.zoom_level as f64 + 7.0) - 1.0);
        let lat = 180.0 / PI
            ...
            egui::SidePanel::right("GPS_r_panel")
                ...
                .show_inside(ui, |ui| {
                    if let Ok(db) = database.lock() {
                        if let Some((gps_data, _)) = db.gps.data.last() {
                    ...
                    ui.heading(
                        egui::RichText::new("Goal Position")
            ...
            egui::CentralPanel::default().show_inside(ui, |ui| {
                ...
                let egui_plot::PlotResponse {
                    ...
                    if let Ok(db) = database.lock() {
                        ...
                        let point: egui_plot::PlotPoints = db
                            .gps
                            .data
                            .iter()
                            .map(|(gps_data, _)| {
                            ...
                            .collect();
                        let line = egui_plot::Line::new("path", point)
                    ...
                    plot_ui.line(
                        egui_plot::Line::new(
                            ...
                            egui_plot::PlotPoints::new(
                                (0..=512)
                                    .map(|i| {
                                        let theta = 2.0 * PI / 512.0 * i as f64;
                                        [
                                            pylon1km_x as f64 + radius * theta.cos(),
                                            pylon1km_y as f64 + radius * theta.sin(),
                                        ]
                                    })
                                    .collect(),
                            ),
                            ...
                            egui_plot::PlotPoints::new(
                                (0..=512)
                                    .map(|i| {
                                        ...
                                        [
                                            chikubushima_x as f64 + radius * theta.cos(),
                                            chikubushima_y as f64 + radius * theta.sin(),
                                        ]
                                    })
                                    .collect(),
                            ),
                            ...
                            egui_plot::PlotPoints::new(
                                (0..=512)
                                    .map(|i| {
                                        let theta = 2.0 * PI / 512.0 * i as f64;
                                        [
                                            okishima_x as f64 + radius * theta.cos(),
                                            okishima_y as f64 + radius * theta.sin(),
                                        ]
                                    })
                                    .collect(),
                            ),
                    ...
                    plot_ui.line(
                        egui_plot::Line::new(
                            ...
                            egui_plot::PlotPoints::new(vec![
                                ...
                                [pylon1km_x as f64, pylon1km_y as f64],
                            ]),
                        )
                        .width(1.0),
                    );
                    ...
                    if let Some((lat, lon)) = self.goal {
                        ...
                        plot_ui.add(
                            egui_plot::Points::new("Goal", vec![[gx as f64, gy as f64]])
                                .color(egui::Color32::from_rgb(255, 0, 0))
                                .name("Goal")
                                .shape(egui_plot::MarkerShape::Diamond)
                                .radius(5.0),
                    ...
                    plot_ui.line(egui_plot::Line::new(
                        ...
                        egui_plot::PlotPoints::new(
                            (0..=10)
                                .map(|i| {
                                    let x =
                                        (i as f64 - 5.0) * 256.0 + x - (self.map_x * 256) as f64;
                                    [x, y as f64]
                                })
                                .collect(),
                        ),
                    ...
                    if self.is_tracking {
                        plot_ui.set_plot_bounds(egui_plot::PlotBounds::from_min_max(
                            [x as f64 - self.area_size, y as f64 - self.area_size],
                            [x as f64 + self.area_size, y as f64 + self.area_size],
                        ));
                    }
